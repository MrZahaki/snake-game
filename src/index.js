...
class GameOver{
  ...
  constructor(context, duration){
      ...
      this.button = new CanvasButton(context, 'Link to github!', this.ctx.canvas.width / 2 - btnWidth / 2, this.ctx.canvas.height * 0.05, btnWidth, 50, '#FF007F');
  }
...
window.onload = function() {
  ...
  function drawGame(){
      ...
      if(gameOver.getState()){
          ...
          if( gameOver.updateState(userHandler, snake) 
              || (()=>{
              for(let i = 0; i < deadlyTrain.length; i++){
                  if(checkDeathCollision(snake, deadlyTrain[i], gameOver)){
                    return true;
                  }
                  return false;
                }
...
if(obj.isEnabled() && snake.checkCollision(obj)){
